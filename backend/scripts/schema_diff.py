...
ADVICE = {
    ...
    "remove_table": "A table the database has and models.py does not describe. NEVER drop it\n"
                    "      on this evidence. Either leave it, or add it to UNMANAGED_TABLES in\n"
                    "      alembic/policy.py with a written reason.",
    "remove_column": "A column the database has and models.py does not declare. NEVER drop it\n"
                     "      on this evidence -- it may hold the only copy of something. Re-declare it\n"
                     "      in models.py, or record it as deliberately unmanaged.",
    "remove_index": "An index the database has and models.py does not declare. Find out where\n"
                    "      it came from before deciding: git log -S '<index name>' --all, and check\n"
                    "      whether an index=True flag was removed from the column without the live\n"
                    "      index being dropped. Then declare it in models.py, or drop it in a\n"
                    "      migration -- but on the origin, not on this line.",
    ...
    "modify_type": "The same column, typed differently on each side. The four this database\n"
                   "      used to report (posts.tags/connections/thumbnail_spec JSON vs JSONB,\n"
                   "      posts.thumbnail_url String vs TEXT) were reconciled in models.py on\n"
                   "      2026-08-28, so a modify_type here is now NEW. Fix the MODELS to match\n"
                   "      production unless there is a reason not to. Never apply an autogenerated\n"
                   "      ALTER TYPE to a populated column without reading it: it rewrites the table.",
