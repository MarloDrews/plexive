...
Read that number carefully before anyone panics or relaxes. 50 alerts is not 50 things shipping in the Android app: a plugin-classpath advisory is a build-time exposure, which is a different and usually much smaller risk than a runtime one, and the same package appears many times because each advisory is its own alert. What the number does establish is the size of what was previously invisible. None of it has been triaged, and triage is a separate decision from this batch.
