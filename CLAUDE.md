...
Because there is no `push:` trigger, `codeql-action/init` emits `##[warning]1 issue was detected with this workflow: Please specify an on.push hook to analyze and see code scanning alerts from the default branch on the Security tab.` on EVERY run, once per matrix leg. It is a static lint on the workflow file, not a report that anything failed, and it is wrong in the narrow sense that scheduled runs do execute on `main` and their analyses are default-branch analyses. The gap it is really pointing at is latency: without `push:`, a merge does not refresh the Security tab, so the baseline is up to a week stale. Adding `push: branches: [main]` would close that and silence the warning, at the cost of a fourth workflow running on every merge. Left as the user's call, not taken silently.
