...
It is closed by `.github/workflows/dependency-submission.yml`, which submits the resolved graph on every push to `main`. Read the Maven coordinate count out of the SBOM rather than trusting the workflow went green, the same way the 1268/0 reading above was taken: `gh api repos/:owner/:repo/dependency-graph/sbom` and count the `pkg:maven/` external refs. What it buys beyond that is an observation still to be made, not a promise. Coordinates in the graph are what alerts are raised against, but no alert exists until a real advisory matches something `mobile-kmp/` actually resolves, and Dependabot opens a pull request only when there is a fix version to move to. A quiet Security tab for `mobile-kmp/` is therefore not by itself evidence that this is or is not working; the coordinate count is.
