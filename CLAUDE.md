...
- Extend ARCHITECTURE.md only when a change establishes something another session would have to know and that no single file states. An ordinary change earns no entry however large it is, and the test is the ROLE of the change rather than its size: a one-line commit can earn an entry and a thousand-line feature can earn none. What an entry must then do to earn its place is in `.claude/rules/architecture-md.md`, delivered to any session that reads the file.
