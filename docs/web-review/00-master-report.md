...
- Coordinated rename task (Plexive to Plexive). DEAD-016 (user-visible strings: title, auth pages, onboarding, author fallbacks in search and stats), DEAD-017 (all plexive_* client storage keys plus the read-old-write-new migration decision), DEAD-018 (non-visible references: comments, User-Agent, temp prefix, gitignore, the PLEXIVE_CONTENT_STRUCTURE.md pointer). M011 (README) and M015 (token constant) are rename touchpoints that land in the batches but should use the final name. The storage-key migration is the risky part: a naive rename logs everyone out and drops interests, likes and saves. This matches the existing project decision that "Plexive" is the final name.
