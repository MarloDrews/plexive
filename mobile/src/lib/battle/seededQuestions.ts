...
import { mockQuestions } from "../train/mockQuestions"

// Deterministic question sequence for a Battle duel.
//
...
// mobile/src/types/train.ts). So the backend only agrees a random integer seed
// for the room; each client feeds that seed into an identical PRNG here and
// derives the same ordered slice of the shared local pool (mockQuestions).
// Same seed in -> same sequence out on both devices.
...
// pool, capped at `count`. Identical on both devices for a given seed.
