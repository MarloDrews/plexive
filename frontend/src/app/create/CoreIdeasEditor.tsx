    ...
    <Accordion title="Core Ideas (6–12)" required defaultOpen>
      ...
      {items.map((ci, i) => (
        <div key={i} className="mb-4 bg-white/[0.04] rounded-2xl p-3">
          ...
          <input type="text" value={ci.title} onChange={(e) => { const n = [...items]; n[i] = { ...n[i], title: e.target.value }; onChange(n) }} placeholder="Concept name..." className={`${inputCls} mb-2`} />
          <label className="text-ink-faint text-xs mb-1 block">Body *</label>
          <textarea value={ci.body} onChange={(e) => { const n = [...items]; n[i] = { ...n[i], body: e.target.value }; onChange(n) }} rows={3} placeholder="Explain the idea..." className={`${inputCls} resize-none mb-2`} />
          <label className="text-ink-faint text-xs mb-1 block">In practice (optional)</label>
          <input type="text" value={ci.in_practice} onChange={(e) => { const n = [...items]; n[i] = { ...n[i], in_practice: e.target.value }; onChange(n) }} placeholder="How to apply this..." className={`${inputCls} mb-2`} />
          <label className="text-ink-faint text-xs mb-1 block">Pull quote (optional)</label>
          <input type="text" value={ci.quote} onChange={(e) => { const n = [...items]; n[i] = { ...n[i], quote: e.target.value }; onChange(n) }} placeholder="A notable quote..." className={inputCls} />
        </div>
      ))}
