...
const TakeawayEditor = memo(function TakeawayEditor({
  ...
  return (
    <Accordion title="Takeaway" required defaultOpen>
      ...
      <select value={value.framing} onChange={(e) => onChange({ ...value, framing: e.target.value as "framework" | "question" })} className={`${inputCls} mb-3`}>
      ...
      <textarea value={value.body} onChange={(e) => onChange({ ...value, body: e.target.value })} rows={3} placeholder="The key thing to take away..." className={`${inputCls} resize-none`} />
    </Accordion>
  )
})
